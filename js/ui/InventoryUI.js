// DOM-based inventory overlay. Toggle with E key.
// Shows 27-slot main inventory + 9-slot hotbar + 2x2 crafting grid.
...
export class InventoryUI {
    constructor(inventory, getAtlas) {
        ...
        this.getAtlas = getAtlas; // function that returns TextureAtlas instance
        this.isOpen = false;

        // Drag state
        this.dragStack = null;

        // 2x2 crafting grid (personal crafting)
        this.craftingSlots = [ItemStack.empty(), ItemStack.empty(), ItemStack.empty(), ItemStack.empty()];
    ...
    _build() {
        ...
        this.container.id = 'inventory-overlay';
        ...
        // Crafting area: 2×2 grid + arrow + output
        const craftArea = document.createElement('div');
        craftArea.style.cssText = 'display:flex; align-items:center; gap:8px; justify-content:center; margin-bottom:4px;';
        ...
        sep.style.cssText = 'height:4px; border-top:1px solid #999;';
        panel.appendChild(sep);

        // Main inventory (27 slots: 3 rows x 9 cols)
        this._mainSlots = [];
        for (let row = 0; row < 3; row++) {
            const rowEl = document.createElement('div');
            rowEl.style.cssText = 'display:flex; gap:2px;';
            for (let col = 0; col < 9; col++) {
                const slotIndex = 9 + row * 9 + col; // slots 9-35
    ...
    _updateCraftingOutput() {
        ...
        const result = findRecipe(grid, 2, 2);
    ...
    _getItemIcon(itemId) {
        ...
        // Block items: try to get from atlas
        if (isBlock(itemId)) {
            const atlas = this.getAtlas();
            if (atlas && atlas._canvas) {
                try {
                    const { BLOCK_TYPES } = { BLOCK_TYPES: window._BLOCK_TYPES };
                    if (BLOCK_TYPES && BLOCK_TYPES[itemId]) {
                        const { col, row } = BLOCK_TYPES[itemId].topUV;
        ...
        // Non-block items or fallback: colored square
    ...
    _drawToolIcon(ctx, def) {
        ...
        ctx.fillStyle = '#8B6914'; // stick
        ctx.fillRect(14, 16, 4, 14); // handle
    ...
    refresh() {
        ...
        // Inventory slots
        const allSlotEls = [...this._hotbarSlots, ...this._mainSlots];
